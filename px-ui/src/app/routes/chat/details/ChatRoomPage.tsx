...
import ChatBubblesSkeleton from "@features/chat/components/ChatBubblesSkeleton";
...
import ChatTitleSection from "@features/chat/components/ChatTitleSection";
...
const ChatRoomPage = () => {
  ...
  const {
    ...
    },
    {
      ...
      onError: (error: GraphqlApiResponse) => {
        ...
        if (err && err.extensions.classification == "NOT_FOUND") {
          return;
        }
  ...
  const { mutate: removeChat } = useRemoveChatMutation(graphqlRequestClient, {
    onSuccess: (data) => {
      if (data) {
        return window.location.replace(Routes.ChatInbox.path);
      }
    },
    onError: (error: GraphqlApiResponse) => {
  ...
  if (
    ...
    return <Navigate to={Routes.ChatInbox.path} />;
  }
      ...
      <Center h={"100%"}>
        <Stack align="center">
          ...
          <Button variant="light" title="Back to conversations">
            <NavLink to={Routes.ChatInbox.path}>Back to conversations</NavLink>
          </Button>
        </Stack>
      </Center>
    );
